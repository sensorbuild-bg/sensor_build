    ...
    <div className={`py-16 md:py-24 ${lang === 'bg' ? 'bg-[#13182c]' : 'bg-white'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {/* Phone Section */}
          <AnimatedDiv className={`rounded-lg p-8 text-center ${lang === 'bg' ? 'bg-[#1a2342]' : 'bg-gray-50'}`}>
          ...
          <AnimatedDiv className={`rounded-lg p-8 text-center ${lang === 'bg' ? 'bg-[#1a2342]' : 'bg-gray-50'}`}>
            ...
            <h2 className={`text-2xl font-semibold mb-4 ${lang === 'bg' ? 'text-white' : 'text-gray-900'}`}>{t.workingHours}</h2>
            <div className={`space-y-2 ${lang === 'bg' ? 'text-white' : 'text-gray-600'}`}>
          ...
          {/* Location Section */}
          <AnimatedDiv className={`rounded-lg p-8 text-center md:col-span-2 ${lang === 'bg' ? 'bg-[#1a2342]' : 'bg-gray-50'}`}>
            <div className="w-16 h-16 bg-[#388644] rounded-full flex items-center justify-center mx-auto mb-4">
              <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <h2 className={`text-2xl font-semibold mb-4 ${lang === 'bg' ? 'text-white' : 'text-gray-900'}`}>{t.location}</h2>
            <p className={lang === 'bg' ? 'text-white' : 'text-gray-600'}>{t.locationText}</p>
          </AnimatedDiv>
        </div>
      </div>
    </div>
