...
export default function HomeClient() {
  ...
  const advantages = isBg
    ? [
        {
          title: "Ясно още от началото",
          text: "Уточняваме обхвата и дейностите преди старта.",
        },
        {
          title: "Технически подход",
          text: "Решенията се съобразяват с реалното състояние на обекта.",
        },
        {
          title: "Фокус върху детайла",
          text: "Целта е завършен, подреден и устойчив резултат.",
        },
      ]
    : [
        {
          title: "Clear from the start",
          text: "We define the scope and required work before starting.",
        },
        {
          title: "Technical approach",
          text: "Solutions are based on the actual condition of the site.",
        },
        {
          title: "Attention to detail",
          text: "The goal is a complete, organized and durable result.",
        },
      ];

  return (
    <div className={`min-h-screen ${isBg ? "bg-[#13182c]" : "bg-white"}`}>
      {/* HERO */}
      <section className="relative min-h-[64svh] md:min-h-[76vh] flex items-center justify-center overflow-hidden px-4 py-14 sm:px-6 md:py-20 lg:px-8">
        ...
        <div className="relative z-10 mx-auto w-full max-w-5xl text-center">
          ...
          <div className="mt-7 flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-3">
            <Link
              href="/contacts"
              className="rounded-xl bg-[#388644] px-7 py-3.5 text-center text-white text-base font-semibold shadow-lg transition hover:bg-[#2d6b35] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
            >
              {copy.primaryCta}
            </Link>
            <Link
              href="/projects"
              className="rounded-xl border border-white/40 bg-black/15 px-7 py-3.5 text-center text-white text-base font-semibold backdrop-blur-sm transition hover:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
            >
              {copy.secondaryCta}
            </Link>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex gap-2 overflow-x-auto pb-1 md:grid md:grid-cols-4 md:overflow-visible md:pb-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {trustItems.map((item) => (
              <div
                key={item}
                className={`shrink-0 rounded-full border px-4 py-2 text-sm font-semibold md:text-center ${
                  isBg
                    ? "border-white/10 bg-[#1a2342] text-white/90"
                    : "border-gray-200 bg-gray-50 text-gray-800"
                }`}
              >
                <span className="mr-2 text-[#62b946]" aria-hidden="true">✓</span>
                {item}
              </div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div className="max-w-3xl">
              <h2 className={`text-3xl md:text-5xl font-noah-bold ${isBg ? "text-white" : "text-gray-900"}`}>
                {copy.servicesTitle}
              </h2>
              <p className={`mt-3 text-base md:text-lg ${isBg ? "text-white/70" : "text-gray-600"}`}>
                {copy.servicesText}
              </p>
            </div>
            <Link
              href="/services"
              className="hidden md:inline-flex items-center gap-2 font-semibold text-[#62b946] hover:underline"
            >
          ...
          <div className="mt-8 flex gap-4 overflow-x-auto snap-x snap-mandatory pb-3 md:grid md:grid-cols-2 lg:grid-cols-3 md:overflow-visible md:pb-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {featuredServices.map((service) => (
              <Link
                key={service.href}
                href={service.href}
                className={`group relative w-[82vw] max-w-[360px] shrink-0 snap-center overflow-hidden rounded-2xl border shadow-lg md:w-auto md:max-w-none ${
                  isBg ? "border-white/10 bg-[#1a2342]" : "border-gray-200 bg-white"
                }`}
              >
                <div className="relative aspect-[16/10] overflow-hidden">
                  <Image
                    src={service.image}
                    alt={`${isBg ? service.titleBg : service.titleEn} – Sensor Build`}
                    fill
                  ...
                  <h3 className="absolute inset-x-0 bottom-0 p-4 text-xl font-semibold text-white">
                    {isBg ? service.titleBg : service.titleEn}
                  </h3>
                </div>

                <div className="flex items-start justify-between gap-3 p-4">
                  <p className={`text-sm leading-relaxed ${isBg ? "text-white/75" : "text-gray-600"}`}>
                    {isBg ? service.descBg : service.descEn}
                  </p>
                  <span className="mt-0.5 shrink-0 text-xl text-[#62b946] transition-transform group-hover:translate-x-1" aria-hidden="true">
                    →
                  </span>
                </div>
              </Link>
          ...
          <div className="mt-5 md:hidden">
            <Link
              href="/services"
              className="inline-flex items-center gap-2 font-semibold text-[#62b946]"
            >
              {copy.allServices} <span aria-hidden="true">→</span>
            </Link>
          </div>
      ...
      <section className={`py-12 md:py-20 ${isBg ? "bg-[#1a2342]" : "bg-white"}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 flex gap-4 overflow-x-auto snap-x snap-mandatory pb-3 md:grid md:grid-cols-2 lg:grid-cols-4 md:overflow-visible md:pb-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {projectCards.map((project) => (
              <Link
                ...
                className="group relative w-[72vw] max-w-[300px] shrink-0 snap-center overflow-hidden rounded-2xl shadow-lg md:w-auto md:max-w-none"
              >
                <div className="relative aspect-[4/3]">
                  <Image
                    src={project.image}
                    alt={`${isBg ? project.titleBg : project.titleEn} – проект на Sensor Build`}
                    fill
                  ...
                  <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-4">
                    <h3 className="text-base md:text-lg font-semibold text-white">
                      {isBg ? project.titleBg : project.titleEn}
                    </h3>
          ...
          <div className="mt-5 md:hidden">
            <Link href="/projects" className="inline-flex items-center gap-2 font-semibold text-[#62b946]">
              {copy.allProjects} <span aria-hidden="true">→</span>
            </Link>
          </div>
        </div>
      ...
      <section className={`py-12 md:py-16 ${isBg ? "bg-[#13182c]" : "bg-gray-50"}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className={`text-center text-3xl md:text-4xl font-noah-bold ${isBg ? "text-white" : "text-gray-900"}`}>
            {copy.whyTitle}
          </h2>
          ...
          <div className="mt-8 grid gap-3 md:grid-cols-3">
            {advantages.map((item) => (
              ...
              >
                <h3 className={`text-lg font-semibold ${isBg ? "text-white" : "text-gray-900"}`}>
                  {item.title}
                </h3>
                <p className={`mt-2 text-sm leading-relaxed ${isBg ? "text-white/70" : "text-gray-600"}`}>
                  {item.text}
                </p>
              </div>
            ))}
      ...
      <section className={`px-4 pb-12 pt-4 md:px-6 md:pb-20 ${isBg ? "bg-[#13182c]" : "bg-gray-50"}`}>
        <div className="relative mx-auto max-w-6xl overflow-hidden rounded-3xl bg-[#388644] px-5 py-9 text-center shadow-xl sm:px-8 md:py-12">
          ...
          <div className="relative z-10 mx-auto max-w-3xl">
            ...
            <p className="mx-auto mt-3 max-w-2xl text-base md:text-lg leading-relaxed text-white/90">
              {copy.finalText}
            </p>
            <div className="mt-6 flex flex-col sm:flex-row justify-center gap-3">
              <Link
                href="/contacts"
                className="rounded-xl bg-white px-7 py-3.5 font-semibold text-[#2d6b35] transition hover:bg-white/90"
              >
                {copy.finalPrimary}
              </Link>
              <Link
                href="/prices"
                className="rounded-xl border border-white/35 bg-white/10 px-7 py-3.5 font-semibold text-white transition hover:bg-white/20"
              >
                {copy.finalSecondary}
              </Link>
            </div>
